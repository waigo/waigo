...
test['reset password'] = {
  beforeEach: function*() {
    ...
    this.basePath = '/user/reset_password';
    ...
    yield this.fetch('/user/forgot_password', {
      method: 'POST',
      headers: {
  ...
  GET: {
    'html': function*() {
      ...
      this.res1.body.should.contain('Reset password');
      this.res1.body.should.contain('<form');
    },
    'json': function*() {
      yield this.getForm({
        query: {
  ...
  'POST': {
    beforeEach: function*() {
      ...
      const cookie = this.res1.headers['set-cookie'];
      const json = JSON.parse(this.res1.body);

      this.submitForm = _.bind(function*(formData, opts) {
        let form = _.extend({
          '__csrf': json.form.fields.__csrf.value
        }, formData);

        this.res2 = yield this.fetch(this.basePath, _.extend({
          method: 'POST',
          headers: {
            cookie: cookie,
          },
    ...
    failure: {
      html: function*() {
        try {
          ...
          body.should.contain('New password: Must be set');
          body.should.contain('Confirm password: Must be set');
        }          
      },
      json: function*() {
        try {
          ...
          body.error.details.confirm_password[0].should.eql('Must be set');
        }
      },
    },
    'do it!': {
      ...
      'password updated': function*() {
        ...
        (yield this.user.isPasswordCorrect('iuter')).should.be.true;
      },
      'send email': function*() {
        yield this.submitForm({
          password: 'iuter',
        ...
        let _call = this.App.mailer.send.getCall(this.App.mailer.send.callCount - 1);

        _.get(_call, 'args.0.to.id').should.eql(this.user.id);
        _.get(_call, 'args.0.subject').should.eql('Your password has been updated');
      },
