...
const _ = require('lodash'),
  ...
  Q = require('bluebird')const test = require(path.join(process.cwd(), 'test', '_base'))(module)const waigo = global.waigovar mapperconst noop = function *() {}test['route mapper'] = {
  beforeEach: function *() {
    this.createAppModules({
      ...
      startupSteps: [],
      shutdownSteps: [],
    })mapper = waigo.load('support/routeMapper')const _createContext = (req, res) => {
      var context = Object.create(this.App.koa.context)var request = context.request = Object.create(this.App.koa.request)var response = context.response = Object.create(this.App.koa.response)context.app = request.app = response.app = thiscontext.req = request.req = response.req = reqcontext.res = request.res = response.res = resrequest.ctx = response.ctx = contextrequest.response = responseresponse.request = requestcontext.state = {}return context}this.ctx = _createContext({}, {
  ...
  'no routes or middleware': function *() {
    yield mapper.setup(this.App, {}, {})this.expect(this.App.koa.router.match('/')).to.be.undefined},
  ...
  'simple routes': {
    ...
    'test - bad method': function *() {
      this.ctx.request.url = '/'this.ctx.request.method = 'PUT'yield this.App.koa.router.call(this.ctx, noop)},
  },
