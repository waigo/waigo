...
test['slack notifier'] = {
  beforeEach: function*() {
    ...
    notifier = waigo.load('support/notifications/slack');
  },
  ...
  'error sending to slack': function*() {
    ...
    yield this.shouldThrow(fn('msg'), 'errorblah');

    this.webhookSpy.should.have.been.calledWith({
      channel: 'ch1',
      username: 'user1',
      icon_emoji: 'emoji1',
      text: 'msg',
  ...
  'string': function*() {
    ...
    _.get(this.webhookSpy.getCall(0), 'args.0.text', '').should.eql('msg');
  },
  ...
  'object': function*() {
    ...
    let data = { dummy: true, abc: 123 };

    yield fn(data);

    _.get(this.webhookSpy.getCall(0), 'args.0.text', '').should.eql(
      JSON.stringify(data)
    );
  },
};
