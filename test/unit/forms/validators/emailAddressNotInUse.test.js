...
const _ = require('lodash'),
  ...
  validationResult = undefinedtest['emailAddressNotInUse'] = {
  beforeEach: function *() {
    yield this.initApp()yield this.startApp({
      startupSteps: ['db', 'models'],
      shutdownSteps: ['db'],
    })yield this.clearDb('User')validator = waigo.load('support/forms/validators/emailAddressNotInUse')this.ctx = {
      App: this.App,
    }},
  ...
  'is not in use': function *() {
    var fn = validator()yield fn(this.ctx, null, 'test@test.com')},

  'in use': function *() {
    var fn = validator()yield this.App.models.User.register({
      username: 'test',
      email: 'test@test.com',
    })
